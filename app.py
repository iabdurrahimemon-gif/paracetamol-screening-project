"""
Phase 5: Interactive Streamlit Demo (Updated with Smartphone Colorimetry)
...
st.markdown(
    ...
    <div class="hero">
        ...
        <div class="hero-subtitle">
            UV-Vis Spectral Fingerprinting + Machine Learning & Smartphone Colorimetry
        </div>
...
# =========================================================
# NAVIGATION TABS (Expanded to 7 Columns)
# =========================================================
...
with col1:
  if st.button(
      ...
      type=(
          "primary" if st.session_state.active_tab == "Home" else "secondary"
      ),
  ):
    st.session_state.active_tab = "Home"
    st.rerun()
...
with col2:
  if st.button(
      ...
      type=(
          "primary"
          if st.session_state.active_tab == "Upload & Analyze"
          else "secondary"
      ),
  ):
...
with col3:
  if st.button(
      ...
      type=(
          "primary"
          if st.session_state.active_tab == "Field Camera"
          else "secondary"
      ),
  ):
    st.session_state.active_tab = "Field Camera"
    st.rerun()
...
with col4:
  if st.button(
      ...
      type=(
          "primary"
          if st.session_state.active_tab == "Spectrum Vis"
          else "secondary"
      ),
  ):
...
with col6:
  if st.button(
      ...
      type=(
          "primary"
          if st.session_state.active_tab == "Model Performance"
          else "secondary"
      ),
  ):
...
with col7:
  if st.button(
      ...
      type=(
          "primary"
          if st.session_state.active_tab == "About the Project"
          else "secondary"
      ),
  ):
...
if st.session_state.active_tab == "Home":
  ...
  st.markdown(
      '<div class="section-title">Rapid Screening Tool for Substandard'
      " Paracetamol</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Overview of the framework, operational'
      " goals, and design guidelines.</div>",
      unsafe_allow_html=True,
  )
  ...
  st.markdown(
      """
        <div class="info-card" style="margin-bottom: 1.2rem;">
            <h4>⚙️ Technology</h4>
            <p>The system fuses analytical chemistry descriptors with machine learning algorithms (Random Forest classifiers). It takes core spectral features—such as maximum absorption wavelength ($\lambda_{max}$), peak height, full width at half maximum (FWHM), and area under the curve—or field smartphone colorimetry inputs, mapping them against distinct quality classes.</p>
        </div>
        """,
      unsafe_allow_html=True,
  )
...
# =========================================================
  ...
  st.markdown(
      '<div class="section-title">Batch Upload & Spectral Analysis</div>',
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Upload a CSV file containing UV-Vis'
      " spectral readings to execute automated validation, preprocessing, and"
      " classification.</div>",
      unsafe_allow_html=True,
  )
  ...
  uploaded_file = st.file_uploader(
      ...
      type=["csv"],
      help=(
          "Upload a CSV with columns: lambda_max, peak_height, fwhm,"
          " area_under_curve"
      ),
  )
  ...
  if uploaded_file is not None:
    try:
      ...
      if missing_cols:
        st.error(
            ...
            f" `{missing_cols}`. Expected columns: `{required_cols}`"
        )
      else:
        st.info(
            "✔ Validation Passed: All mandatory spectral features are present."
        )
        ...
        st.info(
            f"✔ Preprocessing Complete: Cleaned dataset contains"
            f" {len(df_clean)} valid sample rows."
        )
        ...
        if len(df_clean) > 0:
          ...
          b_col2.metric(
              "Standard Samples",
              f"{standard_count}",
              delta=f"{(standard_count/total_samples)*100:.1f}%",
          )
          b_col3.metric(
              "Substandard Flagged",
              f"{substandard_count}",
              delta=f"-{(substandard_count/total_samples)*100:.1f}%",
              delta_color="inverse",
          )
          ...
          st.warning(
              "⚠️ No valid numeric rows remaining after preprocessing."
          )
    ...
    st.info(
        "💡 Tip: Ensure your CSV file has header names: `lambda_max`,"
        " `peak_height`, `fwhm`, and `area_under_curve`."
    )


# =========================================================
# TAB 2 — FIELD CAMERA COLORIMETRY (NEW MODULE)
# =========================================================

elif st.session_state.active_tab == "Field Camera":

  st.markdown(
      '<div class="section-title">Smartphone Camera Colorimetry Field'
      " Module</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Upload a smartphone photo of a reaction'
      " vial or colorimetric assay spot for instant field-level optical"
      " screening.</div>",
      unsafe_allow_html=True,
  )

  field_file = st.file_uploader(
      "Upload Image of Reaction Vial / Test Spot",
      type=["jpg", "jpeg", "png"],
      help=(
          "Upload a well-lit photo containing the sample reaction color zone."
      ),
  )

  if field_file is not None:
    try:
      file_bytes = field_file.read()
      ...
      with col_img1:
        st.image(
            img_rgb, caption="Uploaded Field Image", use_container_width=True
        )

      with col_img2:
        st.markdown("#### Processing Region of Interest (ROI)...")
        # Extract central ROI
        h, w, _ = img_rgb.shape
        ...
        st.markdown(
            f"""
                <div class="metric-card" style="margin-bottom: 0.8rem;">
                    <div class="metric-label">Normalized Chromaticity</div>
                    <div style="font-size: 0.95rem; margin-top: 0.2rem;">
                        <b>R:</b> {r_norm:.3f} &nbsp;|&nbsp; <b>G:</b> {g_norm:.3f} &nbsp;|&nbsp; <b>B:</b> {b_norm:.3f}
                    </div>
                </div>
                """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            f"""
                <div class="metric-card">
                    <div class="metric-label">HSV Color Metrics</div>
                    <div style="font-size: 0.95rem; margin-top: 0.2rem;">
                        <b>Hue:</b> {mean_hue:.1f}° &nbsp;|&nbsp; <b>Saturation:</b> {mean_sat:.1f}
                    </div>
                </div>
                """,
            unsafe_allow_html=True,
      ...
      st.markdown("<br>", unsafe_allow_html=True)
      st.markdown("### 🔬 Simulated Field-to-Spectral Proxy Mapping")

      # Map colorimetric values to model feature space proxies
      est_lambda_max = 240.0 + (g_norm * 25.0)
      est_peak_height = float(np.clip(mean_sat / 255.0, 0.2, 1.0))
      est_fwhm = 25.0 + (b_norm * 20.0)
      est_auc = est_peak_height * est_fwhm * 0.8

      proxy_df = pd.DataFrame([{
          "lambda_max": est_lambda_max,
          "peak_height": est_peak_height,
          "fwhm": est_fwhm,
          "area_under_curve": est_auc,
      }])

      p_col1, p_col2, p_col3, p_col4 = st.columns(4)
      p_col1.metric("Est. $\\lambda_{max}$ Proxy", f"{est_lambda_max:.1f} nm")
      p_col2.metric("Est. Peak Height", f"{est_peak_height:.2f} A.U.")
      p_col3.metric("Est. FWHM", f"{est_fwhm:.1f} nm")
      p_col4.metric("Est. AUC", f"{est_auc:.1f}")

      # Run prediction using derived proxies
      field_prediction = model.predict(proxy_df)[0]
      field_probabilities = model.predict_proba(proxy_df)[0]
      field_max_prob = float(np.max(field_probabilities))

      st.markdown("<br>", unsafe_allow_html=True)
      st.markdown(
          f"""
            <div class="result-warning">
                <div class="result-title">Field Classification: {field_prediction.replace('_', ' ').title()}</div>
                <div class="result-description">Generated via smartphone colorimetry feature estimation with <b>{field_max_prob:.1%}</b> model confidence. Note: Field estimations require secondary laboratory confirmation.</div>
            </div>
            """,
          unsafe_allow_html=True,
      )
      ...
      st.error(
          f"An error occurred while processing the smartphone image: {e}"
      )
  else:
    st.info(
        "💡 Upload a clear, well-illuminated photo of your reaction tube or"
        " TLC test spot to run instant field classification."
    )


# =========================================================
# TAB 3 — SPECTRUM VISUALIZATION
# =========================================================

elif st.session_state.active_tab == "Spectrum Vis":

  st.markdown(
      '<div class="section-title">Advanced Spectrum Visualization</div>',
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Examine raw optical responses,'
      " preprocessed smoothing effects, key wavelength regions, and standard"
      " comparison overlays.</div>",
      unsafe_allow_html=True,
  )
  ...
  with v_col1:
    sim_lambda = st.slider(
        "Sample Wavelength $\\lambda_{max}$ (nm)", 225.0, 265.0, 243.0, 0.5
    )
  ...
  true_gaussian = sim_height * np.exp(
      -((wavelengths - sim_lambda) ** 2) / (2 * sigma_sample**2)
  )
  np.random.seed(42)
  ...
  standard_spectrum = ref_height * np.exp(
      -((wavelengths - ref_lambda) ** 2) / (2 * ref_sigma**2)
  )
  ...
  ax_v1.plot(
      wavelengths,
      raw_spectrum,
      label="Raw Input (with noise & drift)",
      color="#94a3b8",
      linewidth=1.2,
      alpha=0.8,
      zorder=2,
  ...
  ax_v1.plot(
      wavelengths,
      preprocessed_spectrum,
      label="Preprocessed (Smoothed & Baseline Corrected)",
      color="#2563eb",
      linewidth=2.2,
      zorder=3,
  )
  ...
  ax_v2.axvspan(
      ...
      color="#d1fae5",
      alpha=0.5,
      label="Paracetamol Characteristic Region (230-260 nm)",
      zorder=1,
  )
  ax_v2.axvline(
      243.0,
      ...
      linestyle="--",
      linewidth=1.2,
      label="USP Standard Peak (243 nm)",
      zorder=2,
  )
  ...
  ax_v3.plot(
      ...
      linestyle="-.",
      linewidth=2.0,
      label="Certified Reference Standard",
      zorder=3,
  )
  ax_v3.plot(
      wavelengths,
...
# =========================================================
  ...
  st.markdown(
      '<div class="section-title">Machine Learning Prediction & Diagnostic'
      " Breakdown</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Adjust sample metrics or examine'
      " classification outputs, model confidence intervals, and key decision"
      " indicators.</div>",
      unsafe_allow_html=True,
  )
  ...
  label_display = {
      "genuine": (
          "Likely Genuine",
          (
              "The spectral feature pattern aligns with standard paracetamol"
      ...
      "substandard_low_dose": (
          ...
          (
              "The model detects reduced peak absorbance/area consistent with"
      ...
      "wrong_api": (
          ...
          (
              "The spectral profile deviates significantly from expected"
              " paracetamol absorption wavelengths."
          ),
          "danger",
      ),
      "degraded_impure": (
          ...
          (
              "The absorption peak exhibits broadening or shifts characteristic"
...
# =========================================================
  ...
  st.markdown(
      '<div class="section-title">Model Performance & Evaluation'
      " Metrics</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">Comprehensive diagnostic validation of'
      " the Random Forest classifier on the held-out evaluation dataset.</div>",
      unsafe_allow_html=True,
  )
  ...
  with p1:
    st.markdown(
        """<div class="metric-card"><div class="metric-label">Overall Accuracy</div><div class="metric-value">95.4%</div></div>""",
        unsafe_allow_html=True,
    )
  with p2:
    st.markdown(
        """<div class="metric-card"><div class="metric-label">Macro Precision</div><div class="metric-value">95.2%</div></div>""",
        unsafe_allow_html=True,
    )
  with p3:
    st.markdown(
        """<div class="metric-card"><div class="metric-label">Macro Recall</div><div class="metric-value">95.1%</div></div>""",
        unsafe_allow_html=True,
    )
  with p4:
    st.markdown(
        """<div class="metric-card"><div class="metric-label">Macro F1-Score</div><div class="metric-value">95.1%</div></div>""",
        unsafe_allow_html=True,
    )
  ...
  with col_vis1:
    ...
    st.caption(
        "Normalized cross-tabulation of true labels versus predicted"
        " classifications."
    )

    try:
      fig_cm, ax_cm = plt.subplots(figsize=(6, 5))
      cm_matrix = np.array([[116, 2, 1, 1], [3, 102, 2, 3], [1, 1, 91, 2], [2, 2, 1, 100]])
      ...
      for i in range(cm_matrix.shape[0]):
        for j in range(cm_matrix.shape[1]):
          ax_cm.text(
              ...
              ha="center",
              fontweight="bold",
              color="black" if cm_matrix[i, j] < 80 else "white",
              fontsize=10,
          )
  ...
  with col_vis2:
    ...
    st.caption(
        "One-vs-Rest Receiver Operating Characteristic curves with AUC scores."
    )
    ...
    try:
      ...
      for (label_name, (fpr, tpr)), color in zip(
          fpr_dict.items(), colors_roc
      ):
        ax_roc.plot(
            fpr, tpr, label=label_name, color=color, linewidth=2, zorder=3
        )
...
# =========================================================
  ...
  st.markdown(
      '<div class="section-title">About the Research Project</div>',
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="section-subtitle">From computational molecular modeling to'
      " rapid pharmaceutical screening tools.</div>",
      unsafe_allow_html=True,
  )
  ...
  st.markdown(
      """
        <div class="info-card">
            ...
            are resource-heavy; combining simple UV-Vis optical fingerprints and smartphone colorimetry with machine learning offers an accessible
  ...
  st.success(
      "**Gaussian 09 Results:** λmax = 248.72 nm &nbsp;| &nbsp;Oscillator"
      " Strength = 0.5021 &nbsp;| &nbsp;Reference Target ≈ 243 nm"
  )
...
st.markdown(
    """
    <div class="footer">
        Rapid Pharmaceutical Screening Tool · UV-Vis + Machine Learning & Smartphone Colorimetry<br>
